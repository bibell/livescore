 ...
 const fetchData = async () => {
  try {
     ...
     ***/
     ...
     authAxois.get('/admin/adds/alladds/1').then((res)=>{
...
const handleGoogleAdds=async()=>{
 ...
 try {
  ...
  console.error('Error uploading file:', error.response.data);
    ...
    document.querySelector('.allfields').innerHTML='All Fields Requierd'
  }
...
const handleImageUpload = (event) => {
  ...
  return (
    <div className='m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl'>
      <ToastContainer/>
      <Header category="Adds" title="Company Adds"/>
      <Box sx={{
...
<Typography sx={{
   width:'300px'
}}>Your Banner Size Should Be 336x280 PX </Typography><br/>
<TextField type='file' accept="image/*" onChange={(e)=>{
                      handleImageUpload,setFiles(e.target.files[0])}} sx={{
   width:'300px'
}}/>{error && <p style={{ color: 'red' }}>{error}</p>}
      {imagePreview && <img src={imagePreview} alt="Preview" style={{width:'300px',height:'250px'}}/>}<br/><br/>
...
<Box sx={{
   position:{md:'fixed'},
   backgroundColor:'#eeeeee',
   marginLeft:{md:'350px',xs:'10px'},
   width:'300px',
   height:'380px',
   marginTop:{md:'-450px',xs:'50px'},
   overflow:'scroll'

}}>
   <Typography variant='h6' sx={{
    marginLeft:'20px'
   }}>Recent Company Adds Posts</Typography>
   <Box sx={{}}>
     {
      news?.map((item,key)=>{
         return(<Box sx={{
          ...
          boxShadow:'5px 5px 10px #fff'
           ...
           <Typography>{item.expired_date}</Typography>
           <Typography variant='h6' sx={{
             padding:'10px'
           }}>{item.description}</Typography>
           <Button onClick={(item)=>{setOpen(true),setFormData({
            id:item.id,
            description:item.description,
            image:item.image
           }) }} style={{
